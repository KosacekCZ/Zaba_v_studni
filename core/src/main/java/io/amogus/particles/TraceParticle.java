...
public class TraceParticle extends Particle
{
    ...
    public void update(float dt) {
        ...
        if (lifespan < 4 && lifespan > 3) { scale = 1.00f;  alpha = 0.5f; }
        else if (lifespan < 3 && lifespan > 2) { scale = 0.75f; alpha = 0.4f; }
        else if (lifespan < 2 && lifespan > 1) { scale = 0.50f; alpha = 0.3f; }
        else if (lifespan < 1 && lifespan > 0) { scale = 0.25f; alpha = 0.2f; }
